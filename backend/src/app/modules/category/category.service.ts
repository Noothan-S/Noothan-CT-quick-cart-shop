...
async function createCategoryIntoDb(
  payload: Category
  ...
  return {
    status: 201,
    success: true,
    message: `${payload.name} is successfully ${
      isExistInTrash ? "restored from trash" : "created as new category"
    }`,
    data: result,
  };
}

async function updateCategoryIntoDb(
  id: string,
