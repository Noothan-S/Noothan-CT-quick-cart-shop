...
async function responseNewReviewIntoDb(
  user: JwtPayload,
  id: string,
  payload: Partial<VendorResponse>
  ...
  if (
    user.id !== review.product.vendor.user.id &&
...
async function updateReviewResponseIntoDb(
  user: JwtPayload,
  id: string,
  payload: Partial<VendorResponse>
): Promise<IServiceReturn> {
...
async function deleteReviewResponseIntoDb(user: JwtPayload, id: string) {
  ...
  const result = await prisma.vendorResponse.delete({
    where: {
      id,
    },
    include: {
